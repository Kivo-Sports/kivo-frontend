...
import { CalendarClock, Heart, Shield, Trophy, ChevronRight, MapPin } from "lucide-react";
...
function SectionTitle({ icon, title, count }: { icon: typeof Heart; title: string; count?: number }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)", marginBottom: "var(--space-3)" }}>
      <Icon icon={icon} size={18} color="var(--color-brand-primary)" />
      <h2 style={{ margin: 0, fontSize: "clamp(16px, 3vw, 20px)", fontWeight: 700, color: "white" }}>{title}</h2>
      {typeof count === "number" && count > 0 && (
        <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", minWidth: 20, height: 20, borderRadius: "var(--radius-full)", background: "rgba(0,230,118,0.12)", color: "var(--color-brand-primary)", fontSize: "var(--text-xs)", fontWeight: 700, padding: "0 6px" }}>
          {count}
        </span>
      )}
    </div>
...
function EmptyCard({ texto, href, cta }: { texto: string; href: string; cta: string }) {
  return (
    <Card padding="lg" style={{ textAlign: "center", border: "1px dashed rgba(255,255,255,0.1)" }}>
      <p style={{ margin: "0 0 var(--space-3)", color: "var(--color-text-muted)", fontSize: "var(--text-sm)" }}>{texto}</p>
      <Link href={href} style={{ color: "var(--color-brand-primary)", textDecoration: "none", fontSize: "var(--text-sm)", fontWeight: 600 }}>
        {cta} →
      </Link>
    </Card>
  );
...
function TimelineRow({ jogo }: { jogo: TimelineItem }) {
  ...
  return (
    <Link href={`/jogos/${jogo.partidaId}`} style={{ textDecoration: "none" }}>
      ...
      >
        {/* Data */}
        <div style={{ flexShrink: 0, textAlign: "center", minWidth: "54px", padding: "6px 8px", borderRadius: "var(--radius-md)", background: "rgba(0,230,118,0.08)", border: "1px solid rgba(0,230,118,0.2)" }}>
          <div style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--color-brand-primary)", textTransform: "uppercase" }}>{dia}</div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: "6px", minWidth: 0 }}>
              ...
              <span style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: "white" }}>{jogo.timeCasa}</span>
            </span>
            ...
            <span style={{ display: "inline-flex", alignItems: "center", gap: "6px", minWidth: 0 }}>
              ...
              <span style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: "white" }}>{jogo.timeVisitante}</span>
            </span>
          </div>
          <div style={{ marginTop: "4px", display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
            <span style={{ fontSize: "var(--text-xs)", color: "var(--color-text-muted)" }}>{jogo.campeonatoNome}</span>
            <span style={{ fontSize: "10px", color: "var(--color-brand-primary)", background: "rgba(0,230,118,0.1)", borderRadius: "var(--radius-full)", padding: "1px 8px" }}>{jogo.origem}</span>
            {jogo.local && (
              <span style={{ display: "inline-flex", alignItems: "center", gap: "3px", fontSize: "10px", color: "var(--color-text-muted)" }}>
...
function SeguindoModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Seguindo" maxWidth={440}>
          {/* Abas */}
          <div style={{ display: "flex", gap: "4px", marginBottom: "var(--space-3)", padding: "4px", background: "rgba(255,255,255,0.03)", borderRadius: "var(--radius-lg)", border: "1px solid rgba(255,255,255,0.06)" }}>
            {(["times", "campeonatos"] as const).map((k) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setAba(k)}
                  style={{
                    flex: 1, padding: "var(--space-2)", borderRadius: "var(--radius-md)", border: "none", cursor: "pointer",
          ...
          {/* Lista */}
          <div style={{ maxHeight: "min(56vh, 420px)", overflowY: "auto", display: "flex", flexDirection: "column", gap: "var(--space-1)" }}>
            {lista.length === 0 ? (
              <p style={{ margin: 0, padding: "var(--space-4)", textAlign: "center", fontSize: "var(--text-sm)", color: "var(--color-text-muted)" }}>
                Você ainda não segue nenhum {aba === "times" ? "time" : "campeonato"}.
              </p>
            ) : aba === "times" ? (
              times.map((t) => (
                <Link key={t.id} href={`/times/${t.id}`} onClick={onClose} style={{ textDecoration: "none" }}>
                  <div
                    style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", padding: "var(--space-2)", borderRadius: "var(--radius-md)", transition: "background 0.15s" }}
                    onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }}
                  >
                    <Avatar name={t.nome} src={t.logoUrl || undefined} size="sm" />
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <p style={{ margin: 0, fontSize: "var(--text-sm)", fontWeight: 600, color: "white", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.nome}</p>
                      <p style={{ margin: 0, fontSize: "var(--text-xs)", color: "var(--color-text-muted)" }}>{t.cidade} · {t.estado}</p>
                    </div>
                    <Icon icon={ChevronRight} size={16} style={{ color: "var(--color-text-muted)", flexShrink: 0 }} />
                  </div>
                </Link>
              ...
              campeonatos.map((c) => (
                <Link key={c.id} href={`/campeonatos/${c.id}`} onClick={onClose} style={{ textDecoration: "none" }}>
                  <div
                    style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", padding: "var(--space-2)", borderRadius: "var(--radius-md)", transition: "background 0.15s" }}
                    onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }}
                  >
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <p style={{ margin: "0 0 2px", fontSize: "var(--text-sm)", fontWeight: 600, color: "white", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.nome}</p>
                      <Badge variant={obterStatusCampeonato(c.status).variant} size="sm">{obterStatusCampeonato(c.status).label}</Badge>
                    </div>
                    <Icon icon={ChevronRight} size={16} style={{ color: "var(--color-text-muted)", flexShrink: 0 }} />
                  </div>
                </Link>
  ...
  return (
    <div style={{ width: "100%", maxWidth: "1100px", margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: "var(--space-6)" }}>
        ...
        <h1 style={{ margin: "2px 0 0", fontSize: "clamp(24px, 4vw, 34px)", fontWeight: 700, color: "white" }}>
        ...
        <div style={{ display: "flex", gap: "var(--space-5)", marginTop: "var(--space-3)" }}>
          {([
            ...
            >
              <strong style={{ color: "white", fontSize: "var(--text-md)", fontWeight: 700 }}>{s.valor}</strong>
        ...
        {carregandoTimeline ? (
          ...
          <Card padding="lg" style={{ border: "1px dashed rgba(255,255,255,0.1)" }}>
            <p style={{ margin: 0, color: "var(--color-text-muted)", fontSize: "var(--text-sm)" }}>
              Nenhum jogo agendado dos seus favoritos. Favorite times e campeonatos para acompanhar a agenda aqui.
            </p>
          </Card>
        ) : (
          <motion.div initial="initial" animate="animate" style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
            {timeline.map((jogo) => <TimelineRow key={jogo.partidaId} jogo={jogo} />)}
          </motion.div>
        )}
      </div>
