...
import { isTorcedor, getRedirectPathAfterLogin } from "@/lib/auth.utils";
import { Spinner } from "@/components/atoms/Spinner";
import { Button } from "@/components/atoms/Button";

export default function TorcedorLayout({ children }: { children: React.ReactNode }) {
  const { user, isHydrated } = useAppSelector((state) => state.auth);
  const router = useRouter();
  const [countdown, setCountdown] = useState(3);

  const hasAccess = isHydrated && isTorcedor(user?.cargo);
      ...
      >
        ...
        >
          ...
          >
            ...
            <strong style={{ color: "var(--color-text-secondary)" }}>Torcedores</strong>.
