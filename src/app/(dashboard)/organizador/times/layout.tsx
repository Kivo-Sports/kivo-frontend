...
import { isOrganizadorTime, getRedirectPathAfterLogin } from "@/lib/auth.utils";
import { Spinner } from "@/components/atoms/Spinner";
import { Button } from "@/components/atoms/Button";

export default function OrganizadorTimesLayout({ children }: { children: React.ReactNode }) {
  const { user, isHydrated } = useAppSelector((state) => state.auth);
  const router = useRouter();
  const [countdown, setCountdown] = useState(3);

  const hasAccess = isHydrated && isOrganizadorTime(user?.cargo);
      ...
      >
        ...
        >
          ...
          >
            ...
            <strong style={{ color: "var(--color-text-secondary)" }}>Organizadores de Time</strong>.
